...
export default async (req, res, ) => {
    ...
    const name = 'gabriellymoura'
    
    const followers = await octokit.request(`/users/${name}/followers?per_page=100`);
    const followerCount = followers.data.length;
    const stars = await octokit.request(`/users/${name}/repos`);
    const starsCount = stars.data.filter(repo => !repo.fork).reduce((acc, item) =>{
        return acc+item.stargazers_count;
    }, 0);

    const reposStarred = await octokit.request(`/users/${name}/starred`);
